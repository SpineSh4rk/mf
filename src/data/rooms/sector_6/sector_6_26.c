#include "data/rooms/sector_6_rooms_data.h"
#include "macros.h"

const u8 sSector6_26_Scrolls[SCROLL_DATA_SIZE(2)] = {
	26, // Room
	2, // Number of scrolls
	
	// Scroll 0
	2, 16, // X bounds
	2, 11, // Y bounds
	4, 7, // Breakeable block position
	3, // Breakeable block direction
	21, // Breakeable block Y bound extension

	// Scroll 1
	2, 16, // X bounds
	12, 22, // Y bounds
	14, 12, // Breakeable block position
	2, // Breakeable block direction
	2, // Breakeable block Y bound extension
};

const u8 sSector6_26_Clipdata[124] = INCBIN_U8("data/rooms/sector_6/sector_6_26_clipdata.gfx");

const u8 sSector6_26_Bg2[14] = INCBIN_U8("data/rooms/sector_6/sector_6_26_bg2.gfx");

const u8 sSector6_26_Bg1[294] = INCBIN_U8("data/rooms/sector_6/sector_6_26_bg1.gfx");

const u8 sSector6_26_Spriteset0[ENEMY_ROOM_DATA_ARRAY_SIZE(2)] = {
	20, 9, SSP_UNINFECTED_OR_BOSS | SPRITESET_IDX(0),
	ROOM_SPRITE_DATA_TERMINATOR
};

